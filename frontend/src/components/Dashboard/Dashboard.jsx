...
export default function Dashboard({ showToast, onBack, onLoadModel, auth }) {
  ...
  const handlePianoVisibilityToggle = async (seq) => {
    ...
    if (success) {
      setSavedPiano((prev) =>
        prev.map((s) => (s.id === seq.id ? { ...s, is_public: newStatus } : s)),
      );
      showToast(
        `Sequence is now ${newStatus ? "Public" : "Private"}`,
        "success",
      );
    } else {
      showToast("Failed to update visibility", "error");
  ...
  const handleGestureVisibilityToggle = async (config) => {
    ...
    if (success) {
      ...
      showToast(
        `Configuration is now ${newStatus ? "Public" : "Private"}`,
        "success",
      );
    } else {
      showToast("Failed to update visibility", "error");
          ...
          <aside className="user-sidebar">
            ...
            <div className="sidebar-menu">
              ...
              >
                ...
                Saved Models{" "}
                <span className="text-xs bg-[var(--bg2)] px-2 py-0.5 rounded-full ml-auto">
                  {savedModels.length}
                </span>
              </div>
              <div
              ...
              <>
                ...
                <div className="info-cards-grid">
                  {savedModels.length === 0 ? (
                    <div className="col-span-full text-[var(--fg-muted)] italic">
                      No models found.
                    </div>
                  ) : (
                    savedModels.map((model) => (
                      <div key={model.id} className="info-card relative group">
                        ...
                        <div className="card-header">
                          <span
                            className="card-label truncate max-w-[160px]"
                            title={model.name}
                          >
                            {model.name}
                          </span>
                        ...
                        <div className="card-value">
                          {model.class_names.length} Classes •{" "}
                          <span
                            className={
                              model.is_public
                                ? "text-[var(--gold)]"
                      ...
                      >
                        <div className="absolute bottom-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handlePianoVisibilityToggle(seq);
                            }}
                            title={
                              seq.is_public ? "Make Private" : "Make Public"
                            }
                          >
                            {seq.is_public ? (
                              <GlobeAltIcon className="w-4 h-4" />
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDeletePiano(seq.id, seq.name_or_title);
                            }}
                            title="Delete Sequence"
                          >
                            <TrashIcon className="w-4 h-4" />
                        ...
                        <div className="card-header">
                          <span
                            className="card-label truncate max-w-[160px]"
                            title={seq.name_or_title}
                          >
                            {seq.name_or_title || `Sequence ${idx + 1}`}
                          </span>
                      ...
                      >
                        <div className="absolute bottom-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleGestureVisibilityToggle(config);
                            }}
                            title={
                        ...
                        <div className="card-header">
                          <span
                            className="card-label truncate max-w-[160px]"
                            title={config.name_or_title}
                          >
                            {config.name_or_title || `Config ${idx + 1}`}
                          </span>
